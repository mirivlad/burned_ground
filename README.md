...
> **Происхождение.** Игра **вдохновлена Scorched Earth** (DOS, 1991, Wendell Hicken) —
> от неё взяты сам жанр, ощущение боя и подход к арсеналу: ступени снарядов
> (Baby → обычный → Heavy), землеройные и землесыпные заряды, осыпание грунта,
> ветер как поправка к наводке. Код написан с нуля, ресурсы оригинала не
> используются: небо, ландшафт, танки, взрывы и звуки генерируются программно.
>
> Это независимый проект. Он не связан с авторами и правообладателями
> оригинальной игры и не является её продолжением или официальной версией.
> Названия снарядов употребляются как отсылка к первоисточнику.
...
Свободное ПО под **GNU Affero General Public License v3** (только версия 3).
Полный текст — в файле [LICENSE](LICENSE).

Выбор AGPL, а не GPL, здесь осознанный: игра работает как сетевой сервис, и
серверная часть (логика комнат, ИИ ботов, рейтинг) никому не «распространяется» —
её просто крутят на своём хосте. Под GPL форк можно было бы поднять публичным
сервером и не отдать ни строчки. Пункт 13 AGPL закрывает этот случай: если люди
играют с вами по сети, они вправе получить исходники — поэтому ссылка на
репозиторий выведена прямо на экране входа.

Практически это значит: играйте, поднимайте свои серверы, меняйте что хотите.
Но если вы запустили изменённую версию для других — опубликуйте изменения.

**Сторонние компоненты.** Phaser 3 распространяется под MIT и поставляется
в составе образа (`public/vendor/phaser.min.js`), чтобы игра не зависела от CDN;
уведомление о копирайте лежит рядом в
[PHASER-LICENSE.txt](public/vendor/PHASER-LICENSE.txt). Остальные зависимости
(Express, Socket.IO, better-sqlite3) — тоже MIT.
